...
from .sts2 import STS2Environment
...
class Env(object):
    ...
    def calc_local_rewards(self, state_prev, state, roles):
        ...
        for idx_agent in range(self.N_home):
            ...
            if role == 0:
                ...
                if state_prev['control_team'] == 1 and state['control_team'] == 0 and state[
                    'control_index'] == idx_agent:
                    # give_reward = True
                    local_rewards[idx_agent] = 1
            elif role == 2:
                ...
                if state['control_team'] == 0 and state['control_index'] != idx_agent:
                    ...
                    for idx_opponent in range(self.N_away):
                        ...
                        if v_ball_opponent.dot(v_ball_agent) <= 0 or np.linalg.norm(v_ball_opponent) > np.linalg.norm(
                                v_ball_agent):
    ...
    def calc_local_rewards_parameterized(self, state_prev, state, roles):
        ...
        for idx_agent in range(self.N_home):
            ...
            if reward_params[1] != 0:
                ...
                if self.home_action_has_shoot and state_prev['home_score'] == state['home_score'] and state[
                    'control_team'] == 0 and state['control_index'] == idx_agent:
            ...
            if reward_params[5] != 0:
                ...
                if state_prev[
                    'control_team'] == 1 and not self.away_action_has_shoot and not self.away_action_has_pass and state[
                    'control_team'] == 0 and state['control_index'] == idx_agent:
                    local_rewards[idx_agent] += reward_params[5]
            if reward_params[6] != 0:
                # Role: Defensive rebound
                if state_prev['control_team'] == 1 and self.away_action_has_shoot and state['control_team'] == 0 and \
                        state['control_index'] == idx_agent:
                    local_rewards[idx_agent] += reward_params[6]
            if reward_params[7] != 0:
                # Role: Blocked pass
                if state_prev['control_team'] == 1 and self.away_action_has_pass and state['control_team'] == 0 and \
                        state['control_index'] == idx_agent:
        ...
        for idx_role in range(self.N_roles):
            ...
            for idx_agent in range(self.N_home):
                if roles[idx_agent] == idx_role:
                    ...
                    # Goals
                    if state_prev['control_team'] == 0 and state_prev['control_index'] == idx_agent and state[
                        'home_score'] - state_prev['home_score'] > 0:
                        matrix_role_counts[idx_role, 0] += 1
                    # Offensive rebound
                    if self.home_prev_action_has_shoot > 0 and state_prev['home_score'] == state['home_score'] and \
                            state['control_team'] == 0 and state['control_index'] == idx_agent:
                        matrix_role_counts[idx_role, 1] += 1
                    # Shot attempts
                    if self.home_shot_attempt_array[idx_agent] == 1:
                        matrix_role_counts[idx_role, 2] += 1
                    # Made pass
                    if self.home_prev_action_has_pass and state_prev['control_team'] == 0 and state_prev[
                        'control_index'] == idx_agent and state['control_team'] == 0 and state[
                        'control_index'] != idx_agent:
                    ...
                    if state_prev[
                        'control_team'] == 1 and not self.away_action_has_shoot and not self.away_action_has_pass and \
                            state['control_team'] == 0 and state['control_index'] == idx_agent:
    ...
    def process_actions(self, actions_int, roles=None, team='home'):
        ...
        for idx_agent in range(team_count):
            ...
            if action_int in self.list_idx_shoot_pass and (
                    self.control_team != team_idx or self.control_index != idx_agent):
                # action is to shoot or pass and it does not have puck
                # make it do nothing
                action = {'action': actions_array[0],
                          'input': [0.0, 0.0]}
            elif action_int == self.map_agent_selfpassaction[
                idx_agent] and self.control_team == team_idx and self.control_index == idx_agent:
    ...
    def update_puck_state(self):
        ...
        self.p_x, self.p_z, self.p_v_x, self.p_v_z = self.state_json[prefix + '_pos_x'], self.state_json[
            prefix + '_pos_z'], self.state_json[prefix + '_vel_x'], self.state_json[prefix + '_vel_z']
    ...
    def step_until_game_on(self):
        ...
        while self.state_json['current_phase'] != 'GAME_ON':
            ...
            counter += 1
            if counter > 20:
                print("env_wrapper.py : stuck in step_until_game_on() due to current_phase=%s. Manually breaking" %
                      self.state_json['current_phase'])
                break
