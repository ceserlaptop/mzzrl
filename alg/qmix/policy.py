...
class QMIX:
    def __init__(self, conf):
        ...
        # 实例化网络
        self.eval_rnn_net = RNN(input_shape, self.conf.rnn_hidden_dim, self.n_actions).to(self.device)
        self.target_rnn_net = RNN(input_shape, self.conf.rnn_hidden_dim, self.n_actions).to(self.device)

        self.eval_qmix_net = QMixNet(self.state_shape, self.conf.qmix_hidden_dim, self.n_agents).to(self.device)
        self.target_qmix_net = QMixNet(self.state_shape, self.conf.qmix_hidden_dim, self.n_agents).to(self.device)

        self.model_dir = self.conf.model_dir + self.conf.map_name

        if self.conf.load_model:
            if os.path.exists(self.model_dir + '/1_rnn_net_params.pkl'):
                rnn_path = self.model_dir + '/1_rnn_net_params.pkl'
                qmix_path = self.model_dir + '/1_qmix_net_params.pkl'
                map_location = 'cuda:2' if self.conf.cuda else 'cpu'
                self.eval_rnn_net.load_state_dict(torch.load(rnn_path, map_location=map_location))
        ...
        self.target_rnn_net.load_state_dict(self.eval_rnn_net.state_dict())
        self.target_qmix_net.load_state_dict(self.eval_qmix_net.state_dict())

        self.eval_parameters = list(self.eval_qmix_net.parameters()) + list(self.eval_rnn_net.parameters())
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):
        ...
        # 取每个agent动作对应的Q值，并且把最后不需要的一维去掉，因为选择之后最后一维只有一个值了
        ...
        # ------------------------------------------------------ #
        # todo: 注意，这个部分用于调试，表示用一个智能体的观测作为状态输入，正常使用时需要进行修改
        # s_in = s[:, :, 0, :]
        # s_in_ = s_[:, :, 0, :]
        # ------------------------------------------------------ #

        # print("q_evals2 shape: ", q_evals.size()) # [batch_size, max_episode_len, n_agents]
        q_total_eval = self.eval_qmix_net(q_evals, s_)
        ...
        if train_step > 0 and train_step % self.conf.update_target_params == 0:  # 更新目标网络
            self.target_rnn_net.load_state_dict(self.eval_rnn_net.state_dict())
            self.target_qmix_net.load_state_dict(self.eval_qmix_net.state_dict())
    ...
    def get_q_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            q_eval, self.eval_hidden = self.eval_rnn_net(inputs, self.eval_hidden)  # (n_agents, n_actions)
            q_target, self.target_hidden = self.target_rnn_net(inputs_, self.target_hidden)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.eval_rnn_net.state_dict(), self.model_dir + '/' + num + '_rnn_net_params.pkl')
