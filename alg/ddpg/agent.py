...
class PolicyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x*self.max_action  # 缩放到 [-action_bound, action_bound]
        return x
...
class QValueNet(nn.Module):
    ...
    def forward(self, state, action):
        # 拼接状态和动作
        cat = torch.cat([state, action], dim=1)  # [b, n_states + n_actions]
...
# ------------------------------------- #
class DDPG:
    def __init__(self, n_states, n_actions, action_bound, args):
        # 策略网络--训练
        self.actor = PolicyNet(n_states, args.n_hiddens, n_actions, action_bound).to(args.device)
        # 价值网络--训练
        self.critic = QValueNet(n_states, args.n_hiddens, n_actions).to(args.device)
        # 策略网络--目标
        self.target_actor = PolicyNet(n_states, args.n_hiddens, n_actions, action_bound).to(args.device)
        # 价值网络--目标
        self.target_critic = QValueNet(n_states, args.n_hiddens, n_actions).to(args.device)
        ...
        self.sigma = args.sigma  # 高斯噪声的标准差，均值设为0
        self.tau = args.tau  # 目标网络的软更新参数
