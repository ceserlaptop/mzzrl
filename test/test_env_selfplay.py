...
for idx_episode in range(1, N_train+1):
    ...
    while not done:
        ...
        if random_actions:
            ...
            count = 1
            if print_action:
                print("actions_int_h", actions_int_h)
                print("actions_int_a", actions_int_a)
                print('')
        else:
            action = input("Enter actions for HOME agents separated by comma, or -1 for all random:")
            if action == '-1':
                actions_int_h = np.random.randint(0, action_dim, N_home)
            else:
                actions_int_h = list(map(int, action.split(',')))

            action = input("Enter actions for AWAY agents separated by comma, or -1 for all random:")
            if action == '-1':
                actions_int_a = np.random.randint(0, action_dim, N_away)
            else:
                actions_int_a = list(map(int, action.split(',')))
