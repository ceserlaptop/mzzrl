...
class Alg(object):
    ...
    def create_networks(self):
        ...
        with tf.variable_scope("Decoder"):
            self.decoder_out, self.decoder_probs = networks.decoder(self.traj, self.traj_length_downsampled,
        ...
        if self.low_level_alg == 'iql':
            ...
            with tf.variable_scope("Qlow_target"):
                self.Q_low_target = networks.Q_low(self.obs, self.z, self.nn['n_h1_low'], self.nn['n_h2_low'],
                                                   self.l_action)
        ...
        with tf.variable_scope("Mixer_main"):
            self.mixer = networks.Qmix_mixer(self.mixer_q_input, self.state, self.l_state, self.n_agents,
                                             self.nn['n_h_mixer'])
        with tf.variable_scope("Mixer_target"):
            self.mixer_target = networks.Qmix_mixer(self.mixer_target_q_input, self.state, self.l_state, self.n_agents,
                                                    self.nn['n_h_mixer'])
    ...
    def get_assign_target_ops(self):
        ...
        for name, var in map_name_Agent_main.items():
            ...
            list_update_ops.append(
                map_name_Agent_target[name].assign(self.tau * var + (1 - self.tau) * map_name_Agent_target[name]))
        ...
        for name, var in map_name_Mixer_main.items():
            # incremental update of target towards main
            list_update_ops.append(
                map_name_Mixer_target[name].assign(self.tau * var + (1 - self.tau) * map_name_Mixer_target[name]))
        ...
        if self.low_level_alg == 'iac':
            ...
            for name, var in map_name_V_main.items():
                list_update_ops_low.append(
                    map_name_V_target[name].assign(self.tau * var + (1 - self.tau) * map_name_V_target[name]))
            ...
            for name, var in map_name_Qlow_main.items():
                list_update_ops_low.append(
                    map_name_Qlow_target[name].assign(self.tau * var + (1 - self.tau) * map_name_Qlow_target[name]))
    ...
    def train_decoder(self, sess, dataset, step_train, summarize=False, writer=None):
        ...
        if summarize:
            summary, _, decoder_probs = sess.run([self.summary_op_decoder, self.decoder_op, self.decoder_probs],
                                                 feed_dict=feed)
            writer.add_summary(summary, step_train)
        else:
