...
class ReplayBuffer(object):
    ...
    def add(self, data):
        ...
        if self._position >= len(self._buffer):
            ...
            self._buffer[self._position] = data
    ...
    def make_index(self, batch_size):
        return [random.randint(0, len(self._buffer) - 1) for _ in range(batch_size)]

    def make_latest_index(self, batch_size):
        idx = [(self._position - 1 - i) % self._capacity for i in range(batch_size)]
    ...
    def sample(self, batch_size):
        ...
        # else:
        #     idxes = range(0, len(self._buffer))
        # return self._encode_sample(idxes)
